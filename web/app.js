...
async function load() {
  try {
    ...
    $("#main").innerHTML = `<div class="banner">${icon("alert")}<span>The Closet isn't running. Open it from the desktop shortcut, then reload this page.</span></div>`;
    return;
  }
  renderNav();
...
async function poll() {
  ...
  try {
    ...
    renderFoot(v.status);
    if (misses >= 3) { misses = 0; $("#gone")?.remove(); await load(); }
    misses = 0;
    if (v.version !== S.version) await load();
  } catch {
    if (++misses >= 3 && !$("#gone")) {
      const b = document.createElement("div");
      b.id = "gone";
      b.className = "banner";
      b.innerHTML = `${icon("alert")}<span>The Closet has closed. Open it again from the desktop shortcut; this page reconnects on its own.</span>`;
      $("#main").prepend(b);
    }
  }
  polling = false;
...
setInterval(() => api("/api/scan", { method: "POST" }).catch(() => {}), 10000);
...
function renderFoot(st) {
  ...
  $("#railFoot").innerHTML = `
    <div class="row"><span class="pulse ${busy ? "busy" : ""}"></span>
      <span>${busy ? `Reading ${st.done + 1} of ${Math.max(st.total, st.done + 1)}…` : "Dump folder watched"}</span></div>
    ...
    <div class="row"><button type="button" id="rescan">Rescan now</button></div>`;
  $("#rescan").onclick = async () => { await api("/api/scan", { method: "POST" }); toast("Dump folder rescanned"); poll(); };
}
...
function route() {
  ...
  if (S.host === "drawer" && !(q || "").includes("f=")) {
    if (isDirty() && !confirm("Discard unsaved changes to the open form?")) return;
    closeDetail();
  }
  renderNav();
  renderView();
...
function viewOverview() {
  ...
  if (!all.length) {
    $("#main").innerHTML = head("Overview") + banners() + `
      ...
      <p>Drop paperwork PDFs into <b>${esc(S.data.dump_dir)}</b>, named like <b>John Doe_Neg_F10_Uniform Violation.pdf</b>. They show up here within a few seconds.</p></div></div>`;
    return;
  }
...
async function openForm(id, opt = {}) {
  if (S.detail && S.detail.id !== id && isDirty() && !confirm("Discard unsaved changes to the open form?")) return;
  let d;
...
function detailHead(d) {
  const r = d.record || {};
  const title = (r.recipient_list || []).join(", ") || d.filename;
  const a = d.analysis || {};
  const chips = [];
  ...
  return `<header class="d-head"><div style="min-width:0"><h2>${esc(title)}</h2><div class="fn">${esc(d.filename)}</div><div class="chips">${chips.join("")}</div></div>
    <div class="spacer"></div><div class="acts">
      <a class="btn ghost small" href="/api/forms/${d.id}/pdf" target="_blank" rel="noopener">${icon("ext")}Open PDF</a>
...
function sheetPane(d) {
  ...
  return `<div class="pager">
    ...
    <div class="sheet ${S.boxes ? "" : "nobox"}" style="aspect-ratio:${cur.w}/${cur.h}"><img src="/api/forms/${d.id}/page/${cur.page}.png?v=${encodeURIComponent(d.parsed_at || "")}" alt="Page ${cur.page + 1} of ${esc(d.filename)}" loading="lazy">${boxes}</div>
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && S.host === "drawer") requestClose();
...
async function act(a) {
  ...
  if (a === "archive" || a === "unarchive") {
    if (isDirty() && !confirm("Discard unsaved changes to this form?")) return;
    const on = a === "archive";
...
function collectClassYears() {
  ...
  for (const [i, n] of names.entries()) {
    ...
    while (!y) {
      const ans = prompt(`Class year for ${n}?\n(Recipient ${i + 1} of ${names.length}. Leave blank if you don't know it.)`, "");
      if (ans === null) return false;
      if (!ans.trim()) { y = ""; break; }
      y = normYear(ans);
      if (!y) alert(`"${ans.trim()}" isn't a class year. Enter something like 2028.`);
    }
    years.push(y);
  }
...
async function save(markReviewed) {
  const d = S.detail;
  const e = S.edit;
  if (!collectClassYears()) { toast("Save cancelled. Nothing was changed.", true); return; }
  const patch = {};
...
function viewSettings() {
  ...
  $("#main").innerHTML = head("Settings", "How rows are written for the tracker and how forms are read.") + `
    ...
    <section class="panel"><div class="panel-head"><h2>Folders</h2></div><div class="panel-body">
      ${row("Dump folder", "Put every PDF here and leave it there. Nothing is ever moved or changed.", `<div class="path">${esc(S.data.dump_dir)}</div>`)}
      ${row("Metadata folder", "Cached reads, page images, and your edits. Back this folder up.", `<div class="path">${esc(S.data.meta_dir)}</div>`)}
      ${row("Templates", "Blank forms used as the reference. They stay in the dump folder and aren't listed as paperwork.", `<div class="path">${esc(Object.values(s.template_files).join(", "))}</div>`)}
    </div></section></div>`;
  $("#resetAll").onclick = async () => {
    ...
    const msg = `Clear all metadata and read ${plural(n, "form")} again from scratch?
