...
//
// The geometry of every slot is read from the blank template PDFs at runtime (so a replaced
// template is picked up automatically). What lives here is the *meaning* of each slot: the
// template widgets are listed in document order and paired with a semantic key, a human label
// and the section of the form they belong to. Field names on real forms drift; this ordering only
// has to be right for the templates themselves.

// [template field name, semantic key, label, section]
export const F10_SLOTS = [
...
export const FORM_TYPES = {
  F10: {
    ...
    templateFile: "F10_nuked_final.pdf",
    slots: F10_SLOTS,
    // Slots whose absence makes a form untrustworthy ("egregious")
    core: ["recipient_name", "date", "issuer", "narrative", "sig_recipient", "sig_aoc", "sqccf_initials"],
    anchorText: ["REPORT OF CONDUCT", "AFCW"],
  },
  F174: {
    title: "AF Form 174",
    subtitle: "Record of Individual Counseling",
    templateFile: "F174_nuked_final.pdf",
    slots: F174_SLOTS,
    core: ["recipient_name", "reason", "narrative", "issuer", "date", "sig_recipient"],
    anchorText: ["INDIVIDUAL COUNSELING", "174"],
  },
};

// The four routing checks the first sergeant tracks. Each maps to the slot(s) whose presence of
// ink/text means "done" for that form type. A flag with no slot can only be set by hand.
...
export function flagSlots(formType, settings) {
  const base = FLAG_SLOTS[formType] || {};
  const out = Object.fromEntries(Object.entries(base).map(([k, v]) => [k, [...v]]));
  if (formType === "F174") {
    const role = settings.f174_commander_role || "cadet_sqcc_signed";
    if (role in out) out[role] = ["sig_commander"];
  }
  return out;
}
